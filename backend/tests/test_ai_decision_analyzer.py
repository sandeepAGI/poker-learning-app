...
class TestAIDecisionAnalyzer(unittest.TestCase):
    ...
    def test_analyze_decision_match_optimal(self, mock_decision_maker):
        ...
        # Setup mocks for AI decisions
        mock_decision_maker.make_decision.side_effect = [
            "call",  # Conservative
            "raise", # Risk Taker
            "fold",  # Probability-Based (changed to avoid priority conflict)
            "fold"   # Bluffer
        ]
        
        # Patch the _find_optimal_strategy method to return a known value
        with patch.object(self.analyzer, '_find_optimal_strategy', 
                        return_value=("Conservative", 1.0)):
            
            # Analyze a "call" decision (matches Conservative)
            ...
            self.assertEqual(result["optimal_strategy"], "Conservative")
    ...
    def test_analyze_decision_non_optimal(self, mock_decision_maker):
        ...
        with patch.object(self.analyzer, '_find_optimal_strategy', 
            ...
            # Analyze a "raise" decision (matches Risk Taker, not optimal)
            result = self.analyzer.analyze_decision(
                player_id=self.player_id,
    ...
    def test_find_matching_strategy_no_exact(self, mock_decision_maker):
        ...
        self.assertEqual(
            self.analyzer._find_matching_strategy("call", strategy_decisions),
            "Conservative"  # First strategy that matches the closest strength
        )
    ...
    def test_generate_feedback(self):
        ...
        decision_data = {
            ...
            "spr": 4.5,
            "game_state": "flop",
            # Include strategy_decisions to fix the error
            "strategy_decisions": {
                "Conservative": "call",
                "Risk Taker": "raise",
                "Probability-Based": "call",
        ...
        # The exact format may vary and improve over time
        ...
        self.assertIn("medium SPR", feedback)
